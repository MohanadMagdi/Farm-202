...
import { formatArabicNumber, formatWeight, formatArabicDate } from "@/lib/arabic-utils";
import { db, FeedingSchedule, FeedingRecord, InventoryItem, Barn } from "@/lib/firebase-mock";
import {
  Plus,
  Clock,
  Utensils,
...
export default function FeedingPage() {
  ...
  const loadData = async () => {
    try {
      const [schedulesSnapshot, recordsSnapshot, inventorySnapshot, barnsSnapshot] = await Promise.all([
      ...
      setFeedingSchedules(schedulesSnapshot.docs.map(doc => doc.data() as FeedingSchedule));
      setFeedingRecords(recordsSnapshot.docs.map(doc => doc.data() as FeedingRecord));
  ...
  const totalScheduledFeeding = todaySchedules.reduce((sum, schedule) => 
    sum + schedule.entries.reduce((entrySum, entry) => entrySum + entry.qtyKgBarnTotal, 0), 0
  );

  const totalActualFeeding = todayRecords.reduce((sum, record) => sum + record.qtyKg, 0);

  const completionRate = totalScheduledFeeding > 0 
    ? Math.round((totalActualFeeding / totalScheduledFeeding) * 100) 
    : 0;

  const pendingFeedings = todaySchedules.reduce((sum, schedule) => 
    sum + schedule.entries.length, 0
  ) - todayRecords.length;

  // Mock upcoming feeding times
  const upcomingFeedings = [
    { time: "07:00", barn: "الحظيرة الرئيسية - ذكور", feed: "دريس البرسيم", qty: 25 },
    { time: "13:00", barn: "حظيرة الإناث الرئيسية", feed: "تبن القمح", qty: 20 },
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">إجمالي التغذية المجدولة</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">التغذية المنجزة</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">الوجبات المتبقية</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-farm-800">
              {formatWeight(totalActualFeeding / Math.max(1, todayRecords.length))}
            </div>
            <p className="text-xs text-muted-foreground">
              ...
              {upcomingFeedings.map((feeding, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-white rounded">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">{feeding.barn}</div>
        ...
        <TabsContent value="schedules" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                الجداول المخططة لتاريخ {formatArabicDate(new Date(selectedDate))}
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {todaySchedules.map((schedule) => {
                      ...
                      const totalQty = schedule.entries.reduce((sum, entry) => sum + entry.qtyKgBarnTotal, 0);
                      const recordedFeedings = todayRecords.filter(r => r.barnId === schedule.barnId).length;
                      const completed = recordedFeedings >= schedule.entries.length;
                      
                      return (
                        <TableRow key={schedule.id}>
                          <TableCell>{barn?.name || schedule.barnId}</TableCell>
                          <TableCell>{formatArabicNumber(schedule.sessionsPerDay)}</TableCell>
                          <TableCell>
        ...
        <TabsContent value="records" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                الوجبات المسجلة لتاريخ {formatArabicDate(new Date(selectedDate))}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="rounded-md border">
                <Table>
                  ...
                  <TableBody>
                    {todayRecords.map((record) => {
                      ...
                      const feedItem = inventoryItems.find(i => i.id === record.feedItemId);
                    ...
                    return (
                      <div key={item.id} className="flex items-center justify-between">
                        ...
                        <div className="text-left">
                          <div className="font-semibold">{formatWeight(consumed)}</div>
                          <div className="text-sm text-muted-foreground">
                            {formatArabicNumber(todayRecords.filter(r => r.feedItemId === item.id).length)} وجبة
                          </div>
                        </div>
                      </div>
                    );
