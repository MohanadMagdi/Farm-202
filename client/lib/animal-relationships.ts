...
export function validateMotherChildRelationship(
  ...
  if (motherAge < 365) { // Less than 1 year
...
export async function createMotherChildRelationship(
  ...
  try {
    ...
    const validation = validateMotherChildRelationship(motherId, childData.category, animals);
...
export async function updateMotherChildRelationship(
  ...
  try {
    ...
    if (oldMotherId) {
      ...
      if (oldMother) {
        const updatedOffspringIds = (oldMother.offspringIds || []).filter(id => id !== childId);
        const updatedOffspringCount = Math.max(0, (oldMother.offspringCount || 1) - 1);
    ...
    if (newMotherId) {
      const validation = validateMotherChildRelationship(newMotherId, child.category, animals);
      if (!validation.isValid) {
        return { success: false, errors: validation.errors };
      }
 ...
 */
export async function deleteAnimalWithRelationships(animalId: string): Promise<{ success: boolean; errors: string[] }> {
  try {
    const animals = await dataService.animals.getAll();
    ...
    if (animal.motherId) {
      ...
      if (mother) {
        const updatedOffspringIds = (mother.offspringIds || []).filter(id => id !== animal.id);
        const updatedOffspringCount = Math.max(0, (mother.offspringCount || 1) - 1);
 ...
 */
export async function getOffspringForMother(motherId: string): Promise<Animal[]> {
  try {
    const animals = await dataService.animals.getAll();
 ...
 */
export async function getMotherForChild(childId: string): Promise<Animal | null> {
  try {
    const animals = await dataService.animals.getAll();
    const child = animals.find(animal => animal.id === childId);
...
export async function syncAllMotherChildRelationships(): Promise<{ 
  ...
  try {
    ...
    const mothers = animals.filter(animal => animal.sex === 'female' && animal.category === 'female');
    
    for (const mother of mothers) {
      const offspring = animals.filter(animal => animal.motherId === mother.id);
      const offspringIds = offspring.map(child => child.id);
      
      // Update mother if offspring data is inconsistent
      if (
        JSON.stringify(mother.offspringIds?.sort()) !== JSON.stringify(offspringIds.sort()) ||
