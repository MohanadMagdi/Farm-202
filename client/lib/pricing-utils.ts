...
export const DEFAULT_PRICING_FORMULAS: Record<AnimalCategory, PricingFormula> = {
  male: {
    ...
    description: 'تسعير الذكور بناءً على الوزن والعمر (سعر أساسي + وزن × سعر الكيلو)',
    animalCategory: 'male',
 ...
 */
export function calculateFormulaPrice(animal: Animal, formula?: PricingFormula): number {
  ...
  let calculatedPrice = activeFormula.basePrice + (animal.weight * activeFormula.pricePerKg);
  ...
  if (activeFormula.maxWeight && animal.weight > activeFormula.maxWeight) {
    calculatedPrice = activeFormula.basePrice + (activeFormula.maxWeight * activeFormula.pricePerKg);
  }
  ...
  if (activeFormula.ageMultiplier && animal.birthDate) {
    ...
    const ageMultiplier = Math.pow(activeFormula.ageMultiplier, Math.min(ageInMonths, 24)); // Cap at 24 months
...
export function getMarketPrice(animal: Animal): number {
  ...
  const baseMarketPrice = animal.weight * marketRates[animal.category === 'male' ? 'male' : animal.category === 'female' ? 'female' : 'newborn'];
 ...
 */
export function calculateCurrentPrice(animal: Animal, customFormula?: PricingFormula): number {
...
export function getPricingBreakdown(animal: Animal, formula?: PricingFormula): PricingBreakdown {
  const activeFormula = formula || DEFAULT_PRICING_FORMULAS[animal.category];
  ...
  if (activeFormula.ageMultiplier && animal.birthDate) {
    const ageInMonths = calculateAgeInMonths(animal.birthDate);
    const ageMultiplier = Math.pow(activeFormula.ageMultiplier, Math.min(ageInMonths, 24));
    ageBonus = (basePrice + weightPrice) * (ageMultiplier - 1);
  }
  
  const qualityBonus = (basePrice + weightPrice) * ((activeFormula.qualityMultiplier || 1) - 1);
  const seasonalAdjustment = (basePrice + weightPrice + ageBonus + qualityBonus) * ((activeFormula.seasonalMultiplier || 1) - 1);
