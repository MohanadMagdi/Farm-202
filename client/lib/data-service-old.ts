/**
 * Data Service Layer
 * Uses local API instead of Firebase
 */
...
console.log("Data Service Mode: Local API");

// API service adapters
class APIServiceAdapter<T extends { id: string }> {
  ...
  async query(filters: Array<{ field: string; operator: any; value: any }> = []): Promise<T[]> {
    ...
    const response = await fetch(`/api/${this.endpoint}?${params}`);
