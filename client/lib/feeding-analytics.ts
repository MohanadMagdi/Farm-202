...
export function calculateFeedingEfficiencyMetrics(
  ...
  const totalFeedIssued = feedingRecords.reduce((sum, record) => sum + record.quantityServed, 0);
  ...
  const feedConversionRatio = avgDailyGain > 0 ? feedPerAnimal / avgDailyGain : 0;
  ...
  const benchmarkComparison = getBenchmarkComparison(feedConversionRatio, animals);
  
  // Calculate optimization score and recommendations
  const optimizationScore = calculateOptimizationScore(feedConversionRatio, feedPerAnimal, avgDailyGain, animals);
  const recommendations = generateRecommendations(feedConversionRatio, feedPerAnimal, avgDailyGain, animals);
  const potentialSavings = calculatePotentialSavings(totalFeedCost, optimizationScore);
 ...
 * Calculate average daily gain from weight records
 */
export function calculateAverageDailyGain(animals: Animal[], weightRecords: WeightRecord[]): number {
  ...
  animals.forEach(animal => {
    ...
    if (animalWeights.length >= 2) {
      ...
      const daysDiff = Math.max(1, Math.ceil((lastWeight.date.getTime() - firstWeight.date.getTime()) / (1000 * 60 * 60 * 24)));
      ...
      const daysSinceStart = Math.max(1, Math.ceil((new Date().getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)));
      
      // Estimate birth weight for calculation
      const estimatedBirthWeight = animal.category === 'newborn' ? 3.5 : 
                                  animal.category === 'female' ? 25 : 20; // kg
 ...
 */
export function calculateFeedCost(feedingRecords: FeedingRecord[], warehouseItems: WarehouseItem[]): number {
 ...
 */
export function getEfficiencyRating(fcr: number, animals: Animal[]): 'excellent' | 'good' | 'average' | 'poor' | 'critical' {
  if (animals.length === 0 || fcr === 0) return 'poor';
  
  // Get average benchmark based on animal categories
  const categoryCount = animals.reduce((count, animal) => {
 ...
 */
export function calculateOptimizationScore(fcr: number, feedPerAnimal: number, adg: number, animals: Animal[]): number {
  if (animals.length === 0) return 0;
  ...
  const fcrScore = rating === 'excellent' ? 40 : 
                   rating === 'good' ? 30 : 
                   rating === 'average' ? 20 : 
                   rating === 'poor' ? 10 : 0;
  ...
  const feedScore = feedPerAnimal <= avgMaxFeed ? 30 : Math.max(0, 30 - ((feedPerAnimal - avgMaxFeed) * 10));
  score += feedScore;
 ...
 */
export function generateRecommendations(fcr: number, feedPerAnimal: number, adg: number, animals: Animal[]): string[] {
  const recommendations: string[] = [];
  
  if (fcr > 5.0) {
    recommendations.push('معدل تحويل العلف مرتفع جداً - راجع نوعية العلف وتركيبته');
    recommendations.push('تحقق من الحالة الصحية للحيوانات وتطعيماتها');
  }
  
  if (feedPerAnimal > 2.5) {
    recommendations.push('كمية العلف للحيوان الواحد مرتفعة - قم بتقليل الكمية تدريجياً');
 ...
 */
export function calculatePotentialSavings(currentCost: number, optimizationScore: number): number {
...
export function compareBarnEfficiency(
  ...
  const barnComparisons = barns.map(barn => {
    ...
    const barnFeeding = feedingRecords.filter(record => record.barnId === barn.id);
    
    const totalFeed = barnFeeding.reduce((sum, record) => sum + record.quantityServed, 0);
    const feedPerAnimal = barnAnimals.length > 0 ? totalFeed / barnAnimals.length : 0;
  ...
  const avgEfficiency = barnComparisons.reduce((sum, barn) => sum + barn.efficiency, 0) / barnComparisons.length;
  
  barnComparisons.forEach(barn => {
    barn.efficiencyVsAverage = avgEfficiency > 0 ? ((avgEfficiency - barn.efficiency) / avgEfficiency) * 100 : 0;
  });
...
export function generateFeedingTrends(
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const totalFeed = dayRecords.reduce((sum, record) => sum + record.quantityServed, 0);
