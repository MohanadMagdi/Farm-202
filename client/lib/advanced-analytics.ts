...
export async function getMortalityAnalytics(): Promise<MortalityAnalytics> {
  try {
    const mortalityRecords =
      (await dataService.mortalityRecords?.getAll()) || [];
...
export async function getGrowthAnalytics(): Promise<GrowthAnalytics> {
  try {
    ...
    // Category comparison
    const categoryComparison = growthData.reduce(
      (acc, data) => {
        ...
        acc[data.category].averageADG =
          acc[data.category].totalADG / acc[data.category].count;
        return acc;
      },
